exports.DE = {
  GENERAL: {
    FATHERS: 'Schönstatt Patres',
    HOME: 'Home',
    PROFILE: 'Profil',
    FREE_COMMUNITY: 'Freie Gemeinschaft',
    GENERAL_COMMUNITY: 'Offizielle Gemeinschaft',
    ASSIGNMENTS: 'Ämter/Aufgaben',
    SETTINGS: 'Einstellungen',
    SEARCH: 'Suche',
    BULLETIN: 'Informationstafel',
    NO_INTERNET: 'Keine Internetverbindung',
    ERROR: 'Fehler bei der Anfrage',
    EDIT_SUCCESS: 'Bearbeitung erfolgreich',
    PERSON_EDIT_SUCCESS: 'Person erfolgreich aktualisiert',
    CREATE_SUCCESS: 'Erfolgreich erstellt',
    COPY_CLIPBOARD: 'In die Zwischenablage kopiert',
    IDEAL: 'Beschreibung des Ideals',
    BACK: 'Zurück',
    ERROR_403: 'Sie haben nicht die notwendige Erlaubnis, um diese Aktion auszuführen.',
    ERROR_500: 'Unbekannter Fehler.',
    ERROR_400: 'Ungültige Daten.',
  },
  AUTH_SCREEN: {
    IDENTITY_PLACEHOLDER: 'E-Mail Adresse oder Benutzername',
    NEXT: 'Weiter',
    CODE: 'Habe bereits einen Code',
    SEND: 'Eingeben',
    GO_BACK: 'Zurück',
    TOKEN_PLACEHOLDER: 'Code aus der E-Mail',
    EMPTY_IDENTITY: 'Identität ist leer',
    EMPTY_FIELDS: 'Leere Felder',
    ERROR: 'In der Anfrage ist ein Fehler aufgetreten',
    SEND_EMAIL: 'Wir haben Ihnen eine E-Mail gesendet. Bitte auch Spam überprüfen.',
  },
  HOME_SCREEN: {
    REMINDERS: 'Kalender',
    COMMUNITY_PRAYER: 'Gemeinschaftsgebet',
    MISC: 'Verschiedenes',
  },
  FATHER_DETAIL: {
    DEATHDATE: 'Sterbedatum',
    LEAVEDATE: 'Hat die Gemeinschaft verlassen',
    ASSIGNMENTS: 'Ämter / Aufgaben',
    LAST_UPDATE: 'Letzte Aktualisierung',
    CONTACT_INFO: 'Kontaktinformation',
    EMERGENCY_CONTACT: 'Notfallkontakt',
    SAVE_CONTACT: 'Kontakt speichern',
    SAVED_CONTACT: 'Gespeicherter Kontakt',
    EMAIL: 'Email',
    MAIN_CELL_PHONE: 'Haupthandy',
    CURRENT_HOME: 'Aktuelle Adresse',
    PERSONAL_INFO: 'Persönliche Information',
    PAST_HOMES: 'Frühere Adressen',
    FILIATION: 'Filiale',
    HOME: 'Haus',
    RESPONSIBLE_TERRITORY: 'Verantwortliches Gebiet',
    HOME_COUNTRY: 'Heimatland',
    HOME_TERRITORY: 'Heimatgebiet',
    COURSE: 'Kurs',
    GENERATION: 'Generation',
    BIRTHDAY: 'Geburtstag',
    NAMEDAY: 'Namenstag',
    BAPTISM: 'Tauftag',
    POSTULANCY_ADMITTANCE: 'Aufnahme ins Postulat',
    NOVITIATE_START: 'Noviziatsbeginn',
    COMMUNITY_MEMBERSHIP: 'Anfangsdatum der Mitgliedschaft',
    PERPETUAL_CONTRACT: 'Ewigvertragsweihe',
    DIACONATE_ORDINATION: 'Diakonenweihe',
    PRIESTLY_ORDINATION: 'Priesterweihe',
    START_DATE: 'Anfangsdatum',
    END_DATE: 'Enddatum',
    BISHOP_DATE: ' Bischofsweihe',
  },
  TERRITORY_DETAIL: {
    INFORMATION: 'Information',
    TERRITORY_INFO: 'Information zum Gebiet',
    TERRITORY_CHARGE: 'Verantwortliches Gebiet',
    CELEBRATION_DATE: 'Hauptfest des Gebiets',
    ASSIGNMENTS: 'Ämter',
    TERRITORY_FILIATION: 'Filialen des Gebiets',
    RECTOR: 'Rektor',
    MAIN_HOUSE: 'Haupthaus',
    MEMBERS: 'Mitglieder',
    MEMBERS_OF_TERRITORY: 'Mitglieder dieses Herkunftsgebiets',
    PRIESTS: 'Priester',
    STUDENTS: 'Studenten und Novizen',
    DECEASED: 'Verstorben',
  },
  FILIAL_DETAIL: {
    FILIAL_INFO: 'Information zur Filiale',
    TERRITORY: 'Gebiet',
    SUPERIOR: 'Oberer',
    HOUSES: 'Häuser',
    MEMBERS: 'Mitglieder der Filiale',
    MAIN_HOUSE: 'Haupthaus',
    NO_HOUSES: 'Keine Häuser',
  },
  HOUSE_DETAIL: {
    HOUSE_INFO: 'Information zum Haus',
    PHONE: 'Telefon',
    FAX: 'Fax',
    DIOCESE: 'Diözese',
    ADDRESS: 'Adresse',
    POSTAL_ADDRESS: 'Postadresse',
    EMAIL: 'Email',
    MAIN_HOUSE: 'Haupthaus',
    MEMBERS: 'Mitglieder des Hauses',
    FILIAL: 'Filiale',
    WIFI: 'WLAN Passwort',
    PUBLIC_NOTES: 'Öffentliche Notizen',
    ADMIN_NOTES: 'Admin Notizen',
    NO_MEMBERS: 'Keine Mitglieder',
  },
  GENERATION: {
    GENERATION_INFO: 'Information zur Generation',
    CELEBRATION_DATE: 'Hauptfest des Kurses',
    FOUNDATION_DATE: 'Gründungsdatum',
    COURSES: 'Kurse',
    COURSE_LEADER: 'Kursführer',
    WITHOUT_GENERATIONS: 'Ohne Generation',
    NO_COURSE_LEADER: 'Kein Kursleiter',
    NO_COURSES: 'Keine Kurse',
  },
  COURSE: {
    INFORMATION: 'Information zum Kurs',
    GENERATION: 'Generation',
    CELEBRATION_DATE: 'Hauptfest des Kurses',
    CONSECRATION_DATE: 'Datum der Kursweihe',
    LEADER: 'Kursführer',
    NOVITIATE_START: 'Noviziatsbeginn',
    NOVITIATE: 'Noviziatshaus',
    NOVICE_MASTER: 'Novizenmeister',
    SCHOLASTICATE: 'Studentat',
    SCHOLASTICATE_RECTORS: 'Studentatsrektor(en)',
    FIRST_TERTIANSHIP: 'Erstes Terziat',
    FILIATION: 'Terziatshaus',
    MASTER: 'Terziatsmeister',
    SECOND_TERTIANSHIP: 'Zweites Terziat',
    SION_TIME: 'Sionszeit',
    SION_TIME_MASTER: 'Koordinator der Sionszeit',
    STATEMENT: 'Beschreibung des Kursideals',
    MEMBERS: 'Mitglieder des Kurses',
    START_DATE: 'Anfangsdatum',
    END_DATE: 'Enddatum',
  },
  ASSIGNMENTS: {
    TERRITORY: 'Geb.',
    ALL: 'Alle',
    HISTORICAL: ' Hist.',
    FILIATIONS: 'Fil.',
    GENERATIONS: 'Gener.',
    COURSES: 'Kurse',
    GENERATION: 'Generation',
    COURSE: 'Kurs',
  },
  SETTINGS: {
    LOGOUT: 'Ausloggen',
    VERSION: 'Ausführung',
    LANGUAGE: 'Sprache',
  },
  SEARCH: {
    DECEASED: 'Verstorbene anzeigen',
    EX: 'Ex-Mitglieder anzeigen',
    PLACEHOLDER: 'Vorname oder Nachname',
  },
  FATHER_EDIT: {
    TWITTER: 'Twitter',
    INSTAGRAM: 'Instagram',
    SLACK: 'Slack',
    FACEBOOK: 'Facebook',
    SKYPE: 'Skype',
    PHONE1: 'Telefon 1',
    PHONE2: 'Telefon 2',
    PHONE3: 'Telefon 3',
    SAVE: 'Speichern',
    EDIT: 'Bearbeiten',
    EDIT_LIVING: 'Wohnsituation bearbeiten',
    NAME: 'Name',
    LASTNAME: 'Familienname, Nachname',
    FIRSTNAME: 'Vorname',
    LESSTHAN50: 'Weniger als 50 Zeichen',
    LESSTHAN70: 'Weniger als 70 Zeichen',
    LESSTHAN2000: 'Weniger als 2000 Zeichen',
    LESSTHAN255: 'Weniger als 255 Zeichen',
    FRIENDLY_FIRSTNAME: 'Freundlicher Vorname',
    FRIENDLY_LASTNAME: 'Freundlicher Nachname',
    EMAIL: 'Email',
    EMAIL2: 'Email2',
    CELLPHONE: 'Mobiltelefon',
    CELLPHONE_HAS_WA: 'Handy hat WhatsApp',
    CONTACT_NOTES: 'Kontakthinweise',
    DEACON_DATE: 'Diakon Datum',
    PRIEST_DATE: 'Priester Datum',
    BISHOP_DATE: 'Bischof Date',
    DEATH_DATE: 'Todesdatum',
    LEAVE_DATE: 'Datum verlassen',
    EMERGENCY_CONTACT_NAME_1: 'Name des Notfallkontakts 1',
    EMERGENCY_CONTACT_PHONE_1: 'Notfallkontakttelefon 1',
    EMERGENCY_CONTACT_PHONE_2: 'Notfallkontakttelefon 2',
    EMERGENCY_CONTACT_NAME_2: 'Name des Notfallkontakts 2',
    EMERGENCY_RELATION_CONTACT_1: 'Notfallkontaktbeziehung 1',
    EMERGENCY_RELATION_CONTACT_2: 'Notfallkontaktbeziehung 2',
    EX: 'es.',
  },
  LIVING_SITUATION: {
    EDIT_TITLE: 'Aktive Lebenssituation bearbeiten',
    CREATE_TITLE: 'Neue aktive Lebenssituation',
    RESPONSIBLE_TERRITORY: 'Verantwortliches Gebiet:',
    STATUS: 'Status',
    START_DATE: 'Anfangsdatum',
    END_DATE: 'Enddatum',
    PUBLIC_NOTES: 'Öffentliche Notizen',
    ADMIN_NOTES: 'Admin Notizen',
    SAVE: 'Speichern',
    ADD: 'Neue hinzufügen',
    FILIATION: 'Filialen',
    HOUSE: 'Haus',
    ERROR_END_DATE: 'Das Enddatum kann nicht vor dem Anfangsdatum liegen',
  },
  GALLERY: {
    PHOTOS: 'Fotos',
    PHOTO: 'Foto',
    POST_PHOTO: 'Ein Foto veröffentlichen',
    COMMENTS: 'kommentare',
    CHOOSE_PHOTO: 'Wählen Sie ein Foto aus',
    CHOOSE_ANOTHER_PHOTO: 'Wählen Sie ein anderes Foto aus',
    CAPTION: 'Bildbeschriftung',
    UPLOAD_PHOTO: 'Foto hochladen',
    LIKES: 'Gefällt mir',
    SEE_ALL: 'Alles ansehen',
    SEE_MORE: 'Mehr laden',
    SUCCESS: 'Foto veröffentlicht.',
    COMMENT: 'Kommentieren',
    DELETE_PHOTO_TITLE: 'Foto löschen',
    DELETE_COMMENT_TITLE: 'Kommentar löschen',
    DELETE_PHOTO_BODY: 'Sind Sie sich sicher?',
    DELETE_COMMENT_BODY: 'Sind Sie sich sicher?',
    WARNING: '',
    WARNING_COMMENT: '',
    EDIT: 'Bearbeiten',
  },
  ASSIGNMENTS_FORM: {
    TITLE: 'Neues Amt zuordnen',
    TITLE_EDIT: 'Amt bearbeiten',
    ENTITY: '',
    ROLE_LIST: '',
    ROLE: 'Aufgabe',
    PERSON_LIST: '',
    PERSON: 'Person',
    START_DATE: 'Startdatum',
    END_DATE: 'Enddatum',
    SAVE: 'Speichern',
    DELETE: 'Löschen',
    WARNING: '',
    ERROR: 'Bitte alle notwendigen Felder ausfüllen',
    ERROR_END_DATE: 'Enddatum kann nicht vor dem Startdatum sein',
    SUCCESS: 'Daten erfolgreich gespeichert',
    WARNING_ASG: '',
    DELETE_TITLE: 'Amt löschen',
    DELETE_BODY: 'Sind Sie sich sicher?',
  },
  ARCHIVE: {
    ARCHIVE: 'Archiv',
    UNARCHIVE: 'Nicht archivieren',
    EDIT: 'Bearbeiten',
    ARCHIVED: 'Archiviert',
    ERROR: 'Bitte wählen Sie mindestens einen Beitrag aus',
    NO_POSTS: 'Keine archivierten Beiträge vorhanden',
    SUCCESS: 'Daten erfolgreich gespeichert',
  },
};
