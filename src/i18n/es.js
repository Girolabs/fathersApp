exports.ES = {
  GENERAL: {
    FATHERS: 'Padres de Schoenstatt',
    HOME: 'Principal',
    PROFILE: 'Mi perfil',
    FREE_COMMUNITY: 'Comunidad libre',
    GENERAL_COMMUNITY: 'Comunidad oficial',
    ASSIGNMENTS: 'Cargos',
    SETTINGS: 'Configuraciones',
    SEARCH: 'Buscar',
    BULLETIN: 'Recursos & noticias',
    NO_INTERNET: 'No hay conexión de internet',
    ERROR: 'Error en la consulta',
    EDIT_SUCCESS: 'La edición fue exitosa',
    PERSON_EDIT_SUCCESS: 'Persona actualizada exitosamente',
    CREATE_SUCCESS: 'La creación fue exitosa',
    COPY_CLIPBOARD: 'Copiado al portapapeles',
    IDEAL: 'Explicación del ideal de curso',
    BACK: 'Volver',
    ERROR_403: 'No tienes permiso para realizar esta acción.',
    ERROR_500: 'Error desconocido.',
    ERROR_400: 'Datos inválidos.',
  },
  AUTH_SCREEN: {
    IDENTITY_PLACEHOLDER: 'Correo o usuario',
    NEXT: 'Siguiente',
    CODE: 'Ya tengo un código',
    SEND: 'Ingresar',
    GO_BACK: 'Volver',
    TOKEN_PLACEHOLDER: 'Código enviado por email',
    EMPTY_IDENTITY: 'Agrega un correo o usuario',
    EMPTY_FIELDS: 'Hay campos vacios',
    ERROR: 'Ocurrió un error en la consulta',
    SEND_EMAIL: 'Se envió un correo a tu email. Verifica también en spam.',
  },
  HOME_SCREEN: {
    REMINDERS: 'Calendario',
    COMMUNITY_PRAYER: 'Oración de comunidad',
    MISC: 'Otros',
    SELECT_DATE: 'Seleccione una fecha',
    NO_REMINDERS_FOR_DATE: 'No hay recordatorios para esta fecha',
  },
  FATHER_DETAIL: {
    DEATHDATE: 'Fallecido',
    LEAVEDATE: 'Salió de la Comunidad',
    ASSIGNMENTS: 'Encargos',
    LAST_UPDATE: 'Ultima actualización:',
    CONTACT_INFO: 'Información de contacto',
    EMERGENCY_CONTACT: 'Contacto de emergencia',
    SAVE_CONTACT: 'Guardar contacto',
    SAVED_CONTACT: 'El contacto se ha guardado',
    EMAIL: 'Email',
    MAIN_CELL_PHONE: 'Teléfono principal',
    CURRENT_HOME: 'Casa',
    PERSONAL_INFO: 'Información personal',
    PAST_HOMES: 'Situaciones de vivienda',
    FILIATION: 'Filial',
    HOME: 'Casa',
    RESPONSIBLE_TERRITORY: 'Territorio responsable /  Donde trabaja:',
    HOME_COUNTRY: 'País de origen',
    HOME_TERRITORY: 'Territorio de origen',
    COURSE: 'Curso',
    GENERATION: 'Generación',
    BIRTHDAY: 'Cumpleaños',
    NAMEDAY: 'Onomástico',
    BAPTISM: 'Bautismo',
    POSTULANCY_ADMITTANCE: 'Admisión al postulantado',
    NOVITIATE_START: 'Inicio del noviciado',
    COMMUNITY_MEMBERSHIP: 'Pertenencia comunitaria',
    PERPETUAL_CONTRACT: 'Contrato perpetuo',
    DIACONATE_ORDINATION: 'Ordenación diaconal',
    PRIESTLY_ORDINATION: 'Ordenación sacerdotal',
    START_DATE: 'Fecha de inicio',
    END_DATE: 'Fecha de término',
    BISHOP_DATE: 'Ordenación episcopal',
  },
  TERRITORY_DETAIL: {
    INFORMATION: 'Información',
    TERRITORY_INFO: 'Información del territorio',
    TERRITORY_CHARGE: 'Territorio a cargo',
    CELEBRATION_DATE: 'Fecha de celebración',
    ASSIGNMENTS: 'Cargos',
    TERRITORY_FILIATION: 'Filiales del territorio',
    RECTOR: 'Rector',
    MAIN_HOUSE: 'Casa principal',
    MEMBERS: 'Miembros',
    MEMBERS_OF_TERRITORY: 'Miembros de este territorio de origen',
    PRIESTS: 'Sacerdotes',
    STUDENTS: 'Estudiantes y novicios',
    DECEASED: 'Fallecidos',
  },
  FILIAL_DETAIL: {
    FILIAL_INFO: 'Información de filial',
    TERRITORY: 'Territorio',
    SUPERIOR: 'Superior',
    HOUSES: 'Casas',
    MAIN_HOUSE: 'Casa principal',
    MEMBERS: 'Miembros de la filial',
    NO_HOUSES: 'Sin casas',
  },
  HOUSE_DETAIL: {
    HOUSE_INFO: 'Información de la casa',
    PHONE: 'Teléfono',
    FAX: 'Fax',
    DIOCESE: 'Diócesis',
    ADDRESS: 'Domicilio',
    POSTAL_ADDRESS: 'Dirección postal',
    EMAIL: 'Email',
    MAIN_HOUSE: 'Casa principal',
    MEMBERS: 'Miembros de la casa',
    FILIAL: 'Filial',
    WIFI: 'Contraseña wifi',
    PUBLIC_NOTES: 'Notas públicas',
    ADMIN_NOTES: 'Notas de administrador',
    NO_MEMBERS: 'Sin miembros',
  },
  GENERATION: {
    GENERATION_INFO: 'Información de la generación',
    CELEBRATION_DATE: 'Fecha de celebración',
    FOUNDATION_DATE: 'Fecha de fundación',
    COURSES: 'Cursos',
    COURSE_LEADER: 'Jefe de curso',
    WITHOUT_GENERATIONS: 'Sin generación',
    NO_COURSE_LEADER: 'Sin jefe de curso',
    NO_COURSES: 'Sin cursos',
  },
  COURSE: {
    INFORMATION: 'Información del curso',
    GENERATION: 'Generación',
    CELEBRATION_DATE: 'Fecha de celebración',
    CONSECRATION_DATE: 'Fecha de consagración de curso',
    LEADER: 'Jefe de curso',
    NOVITIATE_START: 'Fecha de inicio del noviciado',
    NOVITIATE: 'Noviciado',
    NOVICE_MASTER: 'Maestro de novicios',
    SCHOLASTICATE: 'Estudiantado',
    SCHOLASTICATE_RECTORS: 'Rectores del estudiantado',
    FIRST_TERTIANSHIP: 'Primer terciado',
    FILIATION: 'Filial',
    MASTER: 'Maestro de terciado',
    SECOND_TERTIANSHIP: 'Segundo terciado',
    SION_TIME: 'Tiempo de Sión',
    SION_TIME_MASTER: 'Coordinador de Tiempo de Sión',
    STATEMENT: 'Explicación del ideal',
    MEMBERS: 'Miembros del curso',
    EXMEMBERS: 'Exmiembros',
    START_DATE: 'Fecha de inicio',
    END_DATE: 'Fecha de término',
  },
  ASSIGNMENTS: {
    TERRITORY: 'Terr.',
    ALL: 'Todo',
    HISTORICAL: ' Hist.',
    FILIATIONS: 'Fil.',
    GENERATIONS: 'Gen.',
    COURSES: 'Cursos',
    GENERATION: 'Gen.',
    COURSE: 'Curso',
  },
  SETTINGS: {
    LOGOUT: 'Cerrar sesión',
    VERSION: 'Versión',
    LANGUAGE: 'Idioma',
  },
  SEARCH: {
    DECEASED: 'Mostrar fallecidos',
    EX: 'Mostrar no-miembros',
    PLACEHOLDER: 'Nombre o apellido',
  },
  FATHER_EDIT: {
    TWITTER: 'Twitter',
    INSTAGRAM: 'Instagram',
    FACEBOOK: 'Facebook',
    SLACK: 'Slack',
    SKYPE: 'Skype',
    PHONE1: 'Teléfono 1',
    PHONE2: 'Teléfono 2',
    PHONE3: 'Teléfono 3',
    SAVE: 'Guardar',
    EDIT: 'Editar perfil',
    EDIT_LIVING: 'Editar situación de vivienda',
    NAME: 'Nombre',
    LASTNAME: 'Apellido',
    FIRSTNAME: 'Primer nombre',
    LESSTHAN50: 'Menos de 50 caracteres',
    LESSTHAN70: 'Menos de 70 caracteres',
    LESSTHAN2000: 'Menos de 2000 caracteres',
    LESSTHAN255: 'Menos de 255 caracteres',
    FRIENDLY_FIRSTNAME: 'Nombre amigable',
    FRIENDLY_LASTNAME: 'Apellido amigable',
    EMAIL: 'Correo',
    EMAIL2: 'Correo 2',
    CELLPHONE: 'Teléfono móvil',
    CELLPHONE_HAS_WA: 'El celular tiene WhatsApp',
    CONTACT_NOTES: 'Notas de contacto',
    DEACON_DATE: 'Fecha de diácono',
    PRIEST_DATE: 'Fecha de sacerdote',
    BISHOP_DATE: 'Fecha de obispo',
    DEATH_DATE: 'Fecha de fallecimiento',
    LEAVE_DATE: 'Fecha de partida',
    EMERGENCY_CONTACT_NAME_1: 'Nombre del contacto de emergencia 1',
    EMERGENCY_CONTACT_PHONE_1: 'Teléfono de contacto de emergencia 1',
    EMERGENCY_CONTACT_PHONE_2: 'Teléfono de contacto de emergencia 2',
    EMERGENCY_CONTACT_NAME_2: 'Nombre del contacto de emergencia 2',
    EMERGENCY_RELATION_CONTACT_1: 'Relación de contacto de emergencia 1',
    EMERGENCY_RELATION_CONTACT_2: 'Relación de contacto de emergencia 2',
    EX: 'ej.',
  },
  LIVING_SITUATION: {
    EDIT_TITLE: 'Editar situación de vivienda activa',
    CREATE_TITLE: 'Crear nueva situación de vivienda',
    RESPONSIBLE_TERRITORY: 'Territorio responsable',
    STATUS: 'Status',
    START_DATE: 'Fecha de inicio',
    END_DATE: 'Fecha de fin',
    PUBLIC_NOTES: 'Notas públicas',
    ADMIN_NOTES: 'Notas de administrador',
    SAVE: 'Guardar',
    ADD: 'Agregar nuevo',
    FILIATION: ' Filiales',
    HOUSE: 'Casa',
    ERROR_END_DATE: 'La fecha de fin no puede ser anterior a la fecha de inicio',
  },
  GALLERY: {
    TITLE: 'Galería de fotos',
    NEW: 'Postear una foto',
    PHOTOS: 'Fotos',
    PHOTO: 'Foto',
    POST_PHOTO: 'Publicar una foto',
    COMMENTS: 'comentarios',
    CHOOSE_PHOTO: 'Elige una foto',
    CHOOSE_ANOTHER_PHOTO: 'Elige otra foto',
    CAPTION: 'Pie de foto',
    UPLOAD_PHOTO: 'S u b i r  f o t o',
    LIKES: 'me gusta',
    SEE_ALL: 'Ver todo',
    SEE_MORE: 'Cargar más',
    SUCCESS: 'Datos guardados exitosamente',
    COMMENT: 'Comentar',
    DELETE_PHOTO_TITLE: 'Eliminar foto',
    DELETE_COMMENT_TITLE: 'Eliminar comentario',
    DELETE_PHOTO_BODY: 'Estás seguro?',
    DELETE_COMMENT_BODY: 'Estás seguro?',
    WARNING: 'Estás seguro de eliminar esta foto?',
    WARNING_COMMENT: 'Estás seguro de eliminar este comentario?',
    EDIT: 'Editar',
  },
  ASSIGNMENTS_FORM: {
    TITLE: 'Asignar cargo',
    TITLE_EDIT: 'Editar cargo',
    ENTITY: 'Entidad',
    ROLE_LIST: 'Lista de roles',
    ROLE: 'Rol',
    PERSON_LIST: 'Lista de personas',
    PERSON: 'Persona',
    START_DATE: 'Fecha de inicio',
    END_DATE: 'Fecha de término',
    MARK_NULL: 'Marcar como indefinido',
    MARK_TODAY: 'Marcar como hoy',
    NULL: 'Indefinido',
    SAVE: 'Guardar',
    DELETE: 'Eliminar',
    WARNING: '¿Estás seguro de eliminar esta asignación?',
    ERROR: 'Por favor, complete los campos',
    ERROR_END_DATE: 'La fecha de fin no puede ser anterior a la fecha de inicio',
    SUCCESS: 'Datos guardados exitosamente',
    WARNING_ASG: '¿Estás seguro de eliminar esta asignación?',
    DELETE_TITLE: 'Eliminar asignación',
    DELETE_BODY: '¿Estás seguro?',
  },
  ARCHIVE: {
    ARCHIVE: 'Archivar',
    UNARCHIVE: 'Desarchivar',
    EDIT: 'Editar',
    ARCHIVED: 'Archivados',
    ERROR: 'Seleccione al menos una entrada',
    NO_POSTS: 'No hay entradas archivadas',
    SUCCESS: 'Datos guardados exitosamente',
  },
};
