...
class AuthScreen extends Component {
  ...
  handleSendIdentity = () => {
    ...
    axios
      .get(`users/request-verification-token?identity=${encodeURIComponent(identity)}`, { data: null })
      .then((res) => {
        const status = res.data.status;
        if (status == 'OK') {
          this.setState({ hasToken: true,snackMsg: 'Se envio un correo a tu email',visible: true,loading:false });
        }
      }).catch((e) => {
  ...
  handleSendToken = () => {
    ...
    this.setState({loading:true})
    axios
      .get(`users/login-with-verification-token?identity=${encodeURIComponent(identity)}&token=${token}`, { data: null })
      .then(async (res) => {
        console.log(res)
